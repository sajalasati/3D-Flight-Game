...
Speedbar::Speedbar(float x, float y, float z,int health) {
...
void Speedbar::set_objects(float speed) {
    float l1 = this->length, h1 = this->height, w1 = this->bar_width;
    float l2 = (speed/max_speed)*(l1- 2*w1);
    ...
    this->object3 = create3DObject(GL_TRIANGLES, 6, vertex_buffer_data3, COLOR_GREEN, GL_FILL);
}