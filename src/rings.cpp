...
Ring::Ring(float x, float y, float z, color_t color, float rot, float points) {
...
bounding_box_t Ring::bounding_box(){
    return {this->position.x, this->position.y, this->position.z, 2.0f * (this->radius) , 2.0f * (this->radius), 0.5f};
}
