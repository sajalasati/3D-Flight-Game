...
Display::Display(float x, float y, float z, int type) {
...
void Display::set_objects(int val){
    ...
    if(this->type ==1)
    this->object1 = create3DObject(GL_TRIANGLES, len*6, vertex_buffer_data1, COLOR_DIGIT_WHITE, GL_FILL);
    else
    this->object1 = create3DObject(GL_TRIANGLES, len*6, vertex_buffer_data1, COLOR_BLACK, GL_FILL);

    //digit d2
    len=0; cur=0; moveby += this->length + this->gap;
    for(int i=0;i<=7;++i){
    ...
    if(this->type == 1)
    this->object2 = create3DObject(GL_TRIANGLES, len*6, vertex_buffer_data2, COLOR_DIGIT_WHITE, GL_FILL);
    else
    this->object2 = create3DObject(GL_TRIANGLES, len*6, vertex_buffer_data2, COLOR_BLACK, GL_FILL);
    ...
    if(this->type ==1)
        this->object3 = create3DObject(GL_TRIANGLES, len*6, vertex_buffer_data3, COLOR_DIGIT_WHITE, GL_FILL);
    else
        this->object3 = create3DObject(GL_TRIANGLES, len*6, vertex_buffer_data3, COLOR_BLACK, GL_FILL);
